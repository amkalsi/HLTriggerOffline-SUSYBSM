...
void PlotMaker::fillPlots(const edm::Event& iEvent)
{
  ...
  hL1METx->Fill(theL1METCollection.px());
  hL1METy->Fill(theL1METCollection.py());
  hL1METphi->Fill(theL1METCollection.phi());
  hL1SumEt->Fill(theL1METCollection.etTotal());
  double L1MetSig = theL1METCollection.etMiss() / sqrt(theL1METCollection.etTotal());
  hL1METSignificance->Fill(L1MetSig);
  for(unsigned int i=0; i<l1bits_->size(); i++) {
    if(l1bits_->at(i)) {
      hL1METAfterL1[i]->Fill(theL1METCollection.etMiss());
      hL1METxAfterL1[i]->Fill(theL1METCollection.px());
      hL1METyAfterL1[i]->Fill(theL1METCollection.py());
      hL1METphiAfterL1[i]->Fill(theL1METCollection.phi());
      hL1SumEtAfterL1[i]->Fill(theL1METCollection.etTotal());
      hL1METSignificanceAfterL1[i]->Fill(L1MetSig);
    }
  }
  for(unsigned int i=0; i<hltbits_->size(); i++) {
    if(hltbits_->at(i)) {
      hL1METAfterHLT[i]->Fill(theL1METCollection.etMiss());
      hL1METxAfterHLT[i]->Fill(theL1METCollection.px());
      hL1METyAfterHLT[i]->Fill(theL1METCollection.py());
      hL1METphiAfterHLT[i]->Fill(theL1METCollection.phi());
      hL1SumEtAfterHLT[i]->Fill(theL1METCollection.etTotal());
